...
void printUsage(){
	printf("Usage: m3mp [option] [pos num] [neg num] [test mod]\n");
	printf("Options:\n");
...
void read_problem(const char *filename, struct problem *prob, struct feature_node *x_space, double bias){
	...
	for(i=0; i<prob->l; i++){
		...
		while(1){
			idx = strtok_r(NULL,  ":", &pl);
			val = strtok_r(NULL," \t", &pl);
...
int main(int argc, char *argv[]){
	...
	#pragma omp parallel sections private(index, bias, prob, x_space, start, finish)
	{
	#pragma omp section
	{
	bias = -1;
	...
	for(int i=0; i<neg_num/4+1; ++i){
		for(int j=0; j<pos_num; ++j){
			...
			printf("Thread %d training model %d-%d-%d Time %f\n", omp_get_thread_num(), index, i, j, timeuse);
	...
	char model_file_name[50];
	char train_file_name[50];
	for(int i=neg_num/4+1; i<neg_num/2+1; ++i){
		for(int j=0; j<pos_num; ++j){
			index = i*pos_num+j;
			sprintf(train_file_name, "../%s/data/%s_train_%d_%d_%d", mod, mod, index, i, j);
			sprintf(model_file_name, "../model/%s/%s_model_%d_%d_%d", mod, mod, index, i, j);
	...
	char model_file_name[50];
	char train_file_name[50];
	for(int i=neg_num/2+1; i<neg_num-neg_num/4-1; ++i){
		for(int j=0; j<pos_num; ++j){
			index = i*pos_num+j;
			sprintf(train_file_name, "../%s/data/%s_train_%d_%d_%d", mod, mod, index, i, j);
			sprintf(model_file_name, "../model/%s/%s_model_%d_%d_%d", mod, mod, index, i, j);
	...
	char model_file_name[50];
	char train_file_name[50];
	for(int i=neg_num-neg_num/4-1; i<neg_num; ++i){
		for(int j=0; j<pos_num; ++j){
			index = i*pos_num+j;
			sprintf(train_file_name, "../%s/data/%s_train_%d_%d_%d", mod, mod, index, i, j);
			sprintf(model_file_name, "../model/%s/%s_model_%d_%d_%d", mod, mod, index, i, j);
	...
	char outName[50];
	sprintf(outName, "m3mp_%s_test_data", argv[1]);
